...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        return (start_position,corners_visited)
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if nextPosition in self.corners:
                    ...
                    # 更新角落访问状态
                    visitedCornersList[index] = True

                # 创建新的状态
                nextState = (nextPosition, tuple(visitedCornersList))
                successors.append((nextState, action, 1))  # 成本设为1
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if not foodList:  # 如果没有食物，启发式值为0
        return 0

    # 查找或计算所有食物点的迷宫距离
    maxDistance = 0
    for food in foodList:
        if (position, food) in problem.heuristicInfo:  # 如果已计算，直接使用
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        problem = AnyFoodSearchProblem(gameState)
        from search import breadthFirstSearch  # 确保正确导入函数
        # Use breadth-first search to find the closest dot
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        "*** YOUR CODE HERE ***"
        x, y = state
        return self.food[x][y]  # 检查当前位置是否有食物
